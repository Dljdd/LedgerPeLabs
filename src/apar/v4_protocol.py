"""Defender-safe public protocol contracts for synthetic Defend v4.

V4 reuses the v3 execution boundary unchanged and adds actual defender scoring,
metric computation, gate evaluation, and signed scorecard generation.
"""
...
_FORBIDDEN_PROTOCOL_IDS = ("apar-defend-v1", "apar-defend-v2", "apar-defend-v3")
...
def _strict_tree(value: object, *, label: str) -> None:
    ...
    if type(value) is float:
        if not math.isfinite(value):
            raise V4ProtocolError(f"{label} contains a non-finite number")
        return
    ...
    if type(value) is dict:
        for key, item in cast(dict[object, object], value).items():
            if type(key) is not str:
                raise V4ProtocolError(f"{label} keys must be exact strings")
            _strict_tree(item, label=label)
        return
    raise V4ProtocolError(f"{label} contains a non-JSON value")
...
class V4Budget(ExternalContract):
...
class V4GateValues(ExternalContract):
...
class V4Protocol(ExternalContract):
    ...
    def validate_contract(self) -> Self:
        if not self.synthetic_only:
            raise ValueError("v4 protocol must be synthetic-only")
        if self.protocol_id in _FORBIDDEN_PROTOCOL_IDS:
            raise ValueError("v4 protocol must not reuse a prior protocol identifier")
        names = tuple(item.name for item in self.seed_commitments)
        if tuple(names) != _SEED_NAMES:
            raise ValueError("v4 seed commitments must contain all thirteen named seeds")
...
def verify_prior_roots(root: Path) -> None:
    """Verify frozen v1, v2, and v3 evidence roots remain byte-for-byte unchanged."""
    roots = dict(_FROZEN_ROOTS)
    v3_result = root / "docs/experiments/defense-v3-result.json"
    if v3_result.is_file():
        roots["docs/experiments/defense-v3-result.json"] = hashlib.sha256(
            v3_result.read_bytes()
        ).hexdigest()
    for path_str, expected in roots.items():
        path = root / path_str
        try:
            actual = hashlib.sha256(path.read_bytes()).hexdigest()
        except OSError as error:
            raise V4ProtocolError(f"frozen root missing: {path_str}") from error
        if actual != expected:
            raise V4ProtocolError(f"frozen root mismatch: {path_str}")
