"""Inert, defender-visible protocol contracts for synthetic Defend v2."""
...
def _strict_tree(value: object, *, label: str) -> None:
    ...
    if type(value) is float:
        if not math.isfinite(value):
            raise V2ProtocolError(f"{label} contains a non-finite number")
        return
    ...
    if type(value) is dict:
        for key, item in cast(dict[object, object], value).items():
            if type(key) is not str:
                raise V2ProtocolError(f"{label} keys must be exact strings")
            _strict_tree(item, label=label)
        return
    raise V2ProtocolError(f"{label} contains a non-JSON value")
...
def _strict_json_loads(raw: bytes) -> object:
    if type(raw) is not bytes:
        raise V2ProtocolError("protocol input must be exact bytes")

    def pairs(pairs_value: list[tuple[str, object]]) -> dict[str, object]:
        result: dict[str, object] = {}
        for key, value in pairs_value:
            if key in result:
                raise V2ProtocolError(f"duplicate protocol key: {key}")
            result[key] = value
        return result
    ...
    try:
        value = json.loads(
            ...
            parse_constant=lambda item: (_ for _ in ()).throw(
                V2ProtocolError(f"non-finite protocol value: {item}")
            ),
        )
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise V2ProtocolError("protocol input is not strict UTF-8 JSON") from error
    _strict_tree(value, label="protocol document")
    if _canonical_json_bytes(value) != raw:
        raise V2ProtocolError("protocol input is not canonical JSON")
    return value
...
class PrevalenceStratum(ExternalContract):
    ...
    def allocation_is_exact(self) -> Self:
        if len(self.family_transaction_counts) != 4 or any(
            type(value) is not int or value < 0
            for value in self.family_transaction_counts
        ):
    ...
    @classmethod
    def fixture(
        cls, transaction_count: int = 100, fraud_transaction_count: int = 20
    ) -> PrevalenceStratum:
        ...
        return cls(
            ...
            family_transaction_counts=counts,
        )
...
def _fixture_seed_commitments() -> tuple[SeedCommitment, ...]:
    """Return public deterministic commitments for in-memory fixtures."""
    return tuple(
        SeedCommitment(
            name=name,
            commitment_sha256=hashlib.sha256(
                _canonical_json_bytes({"name": name, "seed": seed})
            ).hexdigest(),
        )
        for name, seed in (
...
class V2Protocol(ExternalContract):
    ...
    def validate_contract(self) -> Self:
        ...
        if not self.fixture_only:
            ...
            actual = tuple((item.name, item.fraud_transaction_count) for item in self.strata)
            if actual != expected:
        ...
        return cls(
            ...
            strata=(
                ...
                PrevalenceStratum(
                    ...
                    family_transaction_counts=(2, 2, 2, 2),
                ),
                PrevalenceStratum(
                    name="high",
                    transaction_count=transaction_count,
                    fraud_transaction_count=12,
                    family_transaction_counts=(3, 3, 3, 3),
                ),
            ),
            budgets=V2Budget(
...
def load_v2_protocol(path: Path) -> V2Protocol:
    try:
        ...
        if protocol.fixture_only:
            raise V2ProtocolError(
                "fixture-only protocol cannot be loaded as preregistration input"
            )
        return protocol
