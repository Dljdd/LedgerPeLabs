...
def test_legacy_evaluator_v2_protocol_import_fails_closed(tmp_path: Path) -> None:
    """The legacy evaluator package is never defender-visible, even for v2 data."""
...
def test_public_v2_protocol_import_remains_admissible(tmp_path: Path) -> None:
    ...
    _write_defender_source(tmp_path, "from apar.v2_protocol import V2Protocol\n")
...
def test_present_public_v2_protocol_must_match_pinned_bytes(tmp_path: Path) -> None:
    ...
    _write_defender_source(tmp_path, "from apar.v2_protocol import V2Protocol\n")
    package = tmp_path / "src/apar"
    package.mkdir(parents=True, exist_ok=True)
    (package / "v2_protocol.py").write_text(
        "V2Protocol = object\n", encoding="utf-8"
    )
