...
OTHER_PAYMENT_ID = "a2a-payment-2"
...
def test_a2a_interleaved_payments_keep_independent_committed_histories() -> None:
    """Catch a history chain incorrectly requiring contiguous engine UUIDs."""
    engine = _engine()
    _schedule(
        engine,
        _initiate(payment_id=PAYMENT_ID, idempotency_key="initiate-a"),
        _initiate(payment_id=OTHER_PAYMENT_ID, idempotency_key="initiate-b"),
        AcceptA2A(PAYMENT_ID),
        AcceptA2A(OTHER_PAYMENT_ID),
        PostA2A(PAYMENT_ID),
        PostA2A(OTHER_PAYMENT_ID),
    )
    ...
    assert engine.ledger.balance("payee") == Decimal("20.00")
    engine.ledger.assert_conserved()


def test_a2a_cross_entity_event_id_substitution_breaks_history_commitment() -> None:
...
def test_a2a_history_commitment_corruption_fails_before_new_effect(
    corruption: str,
) -> None:
